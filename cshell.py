...
pythonMajor = sys.version_info.major  # Ex: 3.
pythonMinor = sys.version_info.minor  # Ex: .12
...
#               Multi-syntax
            ...
            elif answer.startswith ("command ") : command(answer.replace("command " , "" , 1))
...
def update():
    if   shutil.which("apt"):    command("sudo apt update && sudo apt upgrade")
    elif shutil.which("dnf"):    command("sudo dnf update")
    elif shutil.which("pacman"): command("sudo pacman -Syu")
    else:
